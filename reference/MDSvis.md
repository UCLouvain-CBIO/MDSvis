# MDSvis package

`MDSvis` provides a shiny application for interactive visualisation of
Multi Dimensional Scaling (MDS) results.

## See also

[CytoMDS](https://uclouvain-cbio.github.io/CytoMDS/reference/CytoMDS-package.html)

[mdsvis_app](https://uclouvain-cbio.github.io/MDSvis/reference/mdsvis_app.md)

## Author

**Maintainer**: Philippe Hauchamps <philippe.hauchamps@uclouvain.be>
([ORCID](https://orcid.org/0000-0003-2865-1852))

Authors:

- Philippe Hauchamps <philippe.hauchamps@uclouvain.be>
  ([ORCID](https://orcid.org/0000-0003-2865-1852))

- Andrea Vicini <andrea.vicini@uclouvain.be>
  ([ORCID](https://orcid.org/0000-0001-9438-6909))

- Laurent Gatto <laurent.gatto@uclouvain.be>
  ([ORCID](https://orcid.org/0000-0002-1520-2268))

Other contributors:

- Shabnam Zaman <shabnam.zaman@uclouvain.be>
  ([ORCID](https://orcid.org/0000-0001-5399-0130)) \[contributor\]
